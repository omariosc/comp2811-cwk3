...
class AlbumLibrary : public QScrollArea {
 ...
 public slots:
  ...
  void filterForAlbum(int album);  // Filter for videos in an album, then
                                   // display it in VideoLibrary

 private:
  std::vector<AlbumButton*>
      buttons;  // holds the buttons triggering Album Selection
  std::vector<VideoFile*>& videos;
  VideoLibrary*
      library;  // pointer to the VideoLibrary displaying the album videos
  QStackedWidget* toggler;  // Pointer to the stacked widget holding this object
                            // and the library above
